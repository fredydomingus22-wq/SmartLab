...
-- =============================
create or replace function smartlab.set_updated_at()
returns trigger
language plpgsql
as $$
begin
...
create table parameters (
    ...
    input_type parameter_input_type not null default 'number',
    created_at timestamptz not null default now()
);
...
begin
  if not exists (
    select 1 from pg_trigger where tgname = 'set_product_parameters_updated_at'
  ) then
    create trigger set_product_parameters_updated_at
