...
This project depends on public npm packages (Next.js 14, TailwindCSS 3.4, etc.).
To keep `npm install` working even when scoped packages are blocked, lightweight fallbacks for
`@supabase/supabase-js` and `@supabase/ssr` live under [`packages/`](../packages). The local
implementations cover the minimal auth helpers used by the UI so the workspace can be installed
and linted completely offline. When you work in an unrestricted environment you can remove those
folders and reinstall the official SDKs if you prefer.
